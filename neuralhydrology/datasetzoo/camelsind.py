...
class CamelsIND(BaseDataset):
    ...
    def _load_basin_data(self, basin: str) -> pd.DataFrame:
        ...
        timeseries_dir = self.cfg.data_dir / timeseries_dir_name

        if not timeseries_dir.is_dir():
            raise FileNotFoundError(f"Time series directory '{timeseries_dir_name}' not found at {self.cfg.data_dir}")
    ...
    def _load_attributes(self) -> pd.DataFrame:
        ...
        if not attributes_file.is_file():
            raise FileNotFoundError(f"Attribute file '{attribute_file_name}' not found at {self.cfg.data_dir}")

        df = pd.read_csv(attributes_file, index_col="gauge_id")
        df.index = df.index.astype(str)
        
        for basin in self.basins:
            if basin not in df.index:
                raise ValueError(f"Basin {basin} from the basin list is missing from the attribute file.")
        
        for attr in self.cfg.static_attributes:
            if attr not in df.columns:
                raise ValueError(f"Static attribute '{attr}' from config not found in columns of {attributes_file}. "
                                 f"Available columns: {df.columns.to_list()}")
...
def _process_and_split_timeseries(base_path: Path, timeseries_output_dir: Path):
    ...
    merged_df['date'] = pd.to_datetime(
        {'year': merged_df['year'], 'month': merged_df['month'], 'day': merged_df['day']},
        errors='coerce'
    )
    merged_df.dropna(subset=['date'], inplace=True)

    cols_to_drop = ['gauge_id', 'year', 'month', 'day']
    for basin_id, basin_df in tqdm(merged_df.groupby('gauge_id'), desc="Writing time series files"):
    ...
    dynamic_features = [col for col in merged_df.columns if col not in utility_columns]
    
    # Separate into likely inputs and targets for clarity
    potential_targets = ['streamflow']
...
def _process_and_merge_attributes(base_path: Path, attributes_output_file: Path):
    ...
    merged_attributes_df = reduce(
        lambda left, right: pd.merge(left, right, on='gauge_id', how='outer'), 
        dataframes
    )
...
def preprocess_camels_ind_dataset(camels_base_path: str, output_dir: str):
    ...
    if main_output_path.exists():
        raise FileExistsError(
            f"Output directory '{main_output_path}' already exists. "
